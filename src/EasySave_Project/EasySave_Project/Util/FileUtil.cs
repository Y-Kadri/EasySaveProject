...
namespace EasySave_Project.Util
{
    public class FileUtil
    {
        ...
        {
            string directoryPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "easySave", "easySaveSetting");
            string filePath = Path.Combine(directoryPath, "appSetting.json");

            // Vérifie si le fichier existe
            if (!File.Exists(filePath))
            {
                // Si le fichier n'existe pas, retourne false (la modification ne peut pas être faite)
                return false;
            }
            ...
            try
            {
                ...
                // Désérialiser les données pour obtenir l'objet AppSettingDto
                AppSettingDto currentSettings = JsonSerializer.Deserialize<AppSettingDto>(jsonString);
                ...
                // Sérialiser à nouveau les données avec la nouvelle langue
                jsonString = JsonSerializer.Serialize(currentSettings, new JsonSerializerOptions { WriteIndented = true });

                // Écrire les modifications dans le fichier JSON
                File.WriteAllText(filePath, jsonString);

                // Journalise le succès de la modification
                string message = TranslationService.GetInstance().GetText("languageUpdated");
                LogManager.Instance.AddMessage(message);
            ...
            {
                // Journalise l'erreur en cas d'exception
                string errorMessage = TranslationService.GetInstance().GetText("errorUpdatingLanguage") + ex.Message;
                LogManager.Instance.AddMessage(errorMessage);
        ...
        public static bool SettingChangeFormat(EasySave_Library_Log.manager.LogFormatManager.LogFormat format)
        {
            ...
            try
            {
                ...
                currentSettings.logFormat = format;
                ...
                // Journalise le succès de la modification
                string message = TranslationService.GetInstance().GetText("formatUpdated");
                LogManager.Instance.AddMessage(message);
            ...
            {
                // Journalise l'erreur en cas d'exception
                string errorMessage = TranslationService.GetInstance().GetText("errorFormatLanguage") + ex.Message;
                LogManager.Instance.AddMessage(errorMessage);
                return false; // Échec de la modification
            }
        }
        ...
        public static void InitSetting()
        {
            ...
            EnsureDirectoryAndFileExist("appSetting.json");
            
            
            // Initial data to be written in the JSON file
            AppSettingDto data = new AppSettingDto(LanguageEnum.FR,  LogFormatManager.LogFormat.JSON);
        ...
        public static AppSettingDto? GetSetting()
        {
            ...
            {
                return null; // Échec de la modification
            }
        }
    }
}
